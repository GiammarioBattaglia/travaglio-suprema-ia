...
for (const slug of slugs) {
  ...
  const targetRel='/assets/episodes/'+slug+'-speech.jpg';
  ...
  ep.image=targetRel;
  ep.imageAlt='Caricatura satirica con dialogo in fumetti collegati ai personaggi.';
  ...
  ep.dialogue_layout='speech_bubbles';
  await fs.writeFile(episodePath,JSON.stringify(ep,null,2)+'\n');
  console.log('SPEECH_BUBBLE_IMAGE_UPGRADED '+slug+' '+targetRel+' bytes='+result.buffer.length);
}
